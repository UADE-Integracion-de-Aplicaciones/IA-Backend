...
const cargarFacturas = async(sourceFilePath, numero_cuenta, columns) =>{
 ...
 const crear = async ({codigo_pago_electronico, numero_factura, importe, fecha_vencimiento, cuenta}) => {
    const cuenta_id = cuenta.get('id');
    return await facturas.create({
        codigo_pago_electronico, numero_factura, importe, fecha_vencimiento, cuenta_id
    })
};
